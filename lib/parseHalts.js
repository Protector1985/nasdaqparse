...
const processHalt = require("./processHalt");
const csvReader = require("./csvReader");
const moment = require("moment");
const returnMessage = require('./returnMessage')
function parseHalts(xmlString, io) {
  try {
      ...
      parser.parseString(xmlString, async (err, result) => {
        if (err) {
          ...
          if (currentData !== storedData) {
            ...
            // evaluates if trading was resumed
              // index 5 is empty on halt
            io.emit("halt", [[],[],[],[mostRecentHalt[1], 'halt', mostRecentHalt[6]]])
            io.emit("haltData", ["HALT", ...mostRecentHalt])
            if (mostRecentHalt[5].length > 3) {
              io.emit([[],[],[],[mostRecentHalt[1], 'resume', mostRecentHalt[6],mostRecentHalt[5]]])
              io.emit("haltData", ["RESUME", ...mostRecentHalt])
